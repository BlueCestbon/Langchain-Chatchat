...
GET_KEY_FILENAME_PROMPT = HumanMessagePromptTemplate.from_template(
    """\
从query中提取出问题可能答案的关键信息来源，通常是文件名称，不需要回答额外的信息，你的回答必须是从原句中提取词语回答，不要回答额外的内容。

query: 如何在订阅服务中录入订阅信息？
answer: 订阅服务

query: 营销服务系统的安全检查管理如何上传检查附件信息？
answer: 安全检查管理

query: 营销服务系统安全培训管理如何查看培训附件信息？
answer: 安全培训管理

query: 电能替代潜力项目生成的业务介绍是什么？
answer: 电能替代潜力项目

query: 营销服务系统资金流水人工勾对是什么？
answer: 资金流水人工勾对

query: 稽查白名单管理业务的具体描述是什么？
answer: 稽查白名单管理
...
PROMPT_TEMPLATES["knowledge_base_chat"] = {
    "default":
        """
        <指令>你现在是电力行业的专家。根据已知信息用中文回答问题，如果根据已知信息无法获得相关的答案，请说 “根据已知信息无法回答该问题”，不要回答和已知信息不相关的问题，不允许在答案中添加编造成分。</指令>
        <已知信息>{{ context }}</已知信息>、
        <问题>{{ question }}</问题>
        """,
    "yjt":
        """
        <指令>你现在是一名合格的应急厅行政审批安检人员。根据已知信息用中文回答问题，如果根据已知信息无法获得相关的答案，请说 “根据已知信息无法回答该问题”，不要回答和已知信息不相关的问题，不允许在答案中添加编造成分。</指令>
        <已知信息>{{ context }}</已知信息>、
        <问题>{{ question }}</问题>
        """,
    "ele_yingxiao_handbook":
        """
        <指令>你现在是电力行业的开发人员，回答关于操作手册相关的问题。根据已知信息用中文回答问题，如果根据已知信息无法获得相关的答案，请说 “根据已知信息无法回答该问题”，不要回答和已知信息不相关的问题，不允许在答案中添加编造成分。</指令>
        <已知信息>{{ context }}</已知信息>、
        <问题>{{ question }}</问题>
        """,
    "text":
    ...
    # <指令>如果这个问题是和电力行业相关的问题请简洁明了的回答，如果不是，请说“根据已知信息无法回答该问题”。</指令>
        """
        <指令>首先判断这个问题是否和电力行业相关，如果是，请简洁明了的回答，如果不是，请说“根据已知信息无法回答该问题”。</指令>
        <问题>{{ question }}</问题>
        """,
}
